...
use crate::{
  ...
  ptrace::{
    BreakPoint, BreakPointHit, BreakPointStop, InspectError,
    engine::{PhantomUnsend, PhantomUnsync},
    inspect::{read_string, read_string_array},
    tracer::{
      SENTINEL_SIGNAL,
      state::{ProcessState, ProcessStatus},
    },
  },
  seccomp,
  timestamp::Timestamp,
...
use super::{PendingRequest, state::ProcessStateStore};

/// The `!Send + !Sync` part of tracer that should only be used on tracer thread.
///
/// (It is actually `Send` when we haven't start the tracer thread.)
pub(super) struct TracerInner {
  store: RefCell<ProcessStateStore>,
...
impl TracerInner {
  pub fn new(
    ...
    Ok(Self {
      store: RefCell::new(ProcessStateStore::new()),
      breakpoints,
      with_tty,
      mode,
      printer,
      modifier_args,
      filter,
      otel: OtelTracer::new(otel, &baseline)?,
      baseline,
      seccomp_bpf,
      msg_tx,
      user,
